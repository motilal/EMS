...
class Subadmins extends CI_Controller {
    ...
    public function index() {
        ...
        $this->viewData['breadcrumb'] = array('SubAdmin Manager' => 'subadmins', $this->viewData['title'] => '');
        $this->layout->view("subadmin/index", $this->viewData);
    }
    ...
    public function showTableData($data) {
        ...
        if ($data != "") {
            foreach ($data as $key => $row) {
                ...
                $rowData[6] = $this->layout->element('element/_module_status', array('status' => $row->active, 'id' => $row->id, 'url' => "subadmins/changestatus"), true);
                $editUrl = 'subadmins/edit/' . $row->id;
                $deleteUrl = 'subadmins/delete';
                $permissionUrl = 'subadmins/permissions/' . $row->id;
                $rowData[7] = $this->layout->element('element/_module_action', array('id' => $row->id, 'editUrl' => $editUrl, 'deleteUrl' => $deleteUrl, 'permissionUrl' => $permissionUrl), true);
                $resultData[] = $rowData;
            }
        }
        return $resultData;
    ...
    public function add() {
        ...
        if ($this->form_validation->run('add_subadmins') === TRUE) {
            ...
            $additional_data = array(
                ...
                'multi_session' => $this->input->post('multi_session')
            );
            $group = array('3');
        ...
        $this->layout->view("subadmin/add", $this->viewData);
    }
    ...
    public function permissions($id = null) {
        ...
        if ($this->input->post()) {
            ...
            if (!empty($permission_array)) {
                ...
                $this->db->where('id', $user_id)->set('update_setting_flag', 1)->update('users');
        ...
        $this->viewData['breadcrumb'] = array(
            'SubAdmin Manager' => 'subadmins', $this->viewData['title'] => '');
        $this->layout->view("subadmin/permissions", $this->viewData);
    }
