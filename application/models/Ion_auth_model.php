...
class Ion_auth_model extends CI_Model {
     ...
     * @author    Mathew
     */
    public function login($identity, $password, $remember = FALSE, $authentication = TRUE) {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, two_step_authentication,multi_session, last_login')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                if ($user->multi_session == 0) {
                    $this->DeleteSessionDuplicates($user->id, $user->email);
                }
                if ($user->two_step_authentication == 1 && $authentication == TRUE) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->session->set_userdata('_auth_password', $plain_password);
                    $this->TwoStepAuthEmail($identity);
                    return FALSE;
                }
                if ($user->active == 0) {
                    $this->trigger_events('post_login_unsuccessful');
    ...
    private function DeleteSessionDuplicates($userid = "", $email = "") {
        $user_id_rejex = '.*"user_id";|s:[0-9]+:".*' . $userid . '.*".*';
        $email_rejex = '.*"email";|s:[0-9]+:".*' . $email . '.*".*';
        $condition = array("data REGEXP '$user_id_rejex'" => NULL, "data REGEXP '$email_rejex'" => NULL);
        $this->db->where($condition)->delete('ci_sessions');
    }

    private function TwoStepAuthEmail($identity) {
        if ($identity != "" && $this->identity_column == 'email') {
            $this->load->helper('string');
            $this->load->library('encryption');
            $email = $identity;
            $code = random_string('numeric', 6);
            $this->db->where('email', $email)->update('users', array('authentication_code' => $code));
            $this->load->helper('email_helper');
            $replaceFrom = array('{name}', '{code}');
            $replaceTo = array($email, $code);
            sendMailByTemplate('two-step-authentication', $replaceFrom, $replaceTo, $email);
            $enc_email = base64_encode($identity);
            redirect("admin/auth/two_step_auth_login/$enc_email");
        }
    }
