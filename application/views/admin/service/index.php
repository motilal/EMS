                        ...
                        <?php if ($result->num_rows() > 0) { ?>
                            <?php foreach ($result->result() as $key => $row): ?> 
                                <tr id="row_<?php echo $row->id; ?>">
                                    ...
                                    <td>
                                        <?php echo $this->layout->element('admin/element/_module_status', array('status' => $row->is_active, 'id' => $row->id, 'url' => "admin/services/changestatus", 'permissionKey' => "service-status"), true); ?>
                                    </td>
                                    <td>  
                                        <?php echo $this->layout->element('admin/element/_module_action', array('id' => $row->id, 'editUrl' => "admin/services/manage", 'deleteUrl' => 'admin/services/delete', 'editPermissionKey' => 'service-edit', 'deletePermissionKey' => 'service-delete'), true); ?>
                                    </td>  
                                </tr>
                            <?php endforeach; ?>
                        <?php } ?> 
...
<div class="modal fade" id="modal-manage">
    <div class="modal-dialog">
        <div class="modal-content">
            <?php echo form_open('admin/services/manage', array("id" => "manage-form", "method" => "post")); ?>
...
<script>
    ...
    $(document).on('click', 'a.edit-row', function (e) {
        ...
        var id = $(this).data('id');
        $.ajax({
            url: '<?php echo site_url('admin/services/getDetailAjax'); ?>',
            type: "POST",
            data: {id: id},
            success: function (res)
            {
